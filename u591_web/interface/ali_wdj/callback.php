...
if(!$result){
    ...
    $sql=$sql." VALUES (103,$pay_id,'$PayName','$server_id','$PayMoney','$orderId','$dwFenBaoID','$Add_Time','1','$game_id','$clienttype')";
    ...
    if($orderStatus=='S'){
        $rpCode =1;
        $isPay = 0;
        ChangPayLog($orderId, $rpCode,$pay_id, $PayMoney);
    ...
    sendTongjiData($gameId,$pay_id,$server_id,$dwFenBaoID,0,$PayMoney,$orderId,1,$tjAppId,$isPay);
    exit("SUCCESS");
}

function ChangPayLog($OrderID,$rpCode,$PayID,$PayMoney){
    $conn = SetConn(88);
