...
$get = serialize($_GET);
write_log(ROOT_PATH."log","haima_callback_ios_log_","post=$post,get=$get, ".date("Y-m-d H:i:s")."\r\n");
...
if(!isset($userParamArr[0]) || !isset($userParamArr[1]) || !isset($userParamArr[2])){
    write_log(ROOT_PATH."log","haima_callback_ios_error_","prams error. post=$post,get=$get, ".date("Y-m-d H:i:s")."\r\n");
    exit('fail');
}
...
if($sign != $signVerify){
    write_log(ROOT_PATH."log","haima_callback_ios_error_","sign error,signStr=$signStr, post=$post,get=$get, ".date("Y-m-d H:i:s")."\r\n");
    exit('fail');
}
...
if($conn == false){
    write_log(ROOT_PATH."log","haima_callback_ios_error_","post=$post,get=$get,msg=account mysql error, ".date("Y-m-d H:i:s")."\r\n");
    exit('error');
}
...
if(!$result_account['NAME']){
    write_log(ROOT_PATH."log","haima_callback_ios_error_", "account is not exist! post=$post,get=$get, ".date("Y-m-d H:i:s")."\r\n");
    exit('fail');
}else{
...
if($result_count['id']){
    write_log(ROOT_PATH."log","haima_callback_ios_error_", "order is exist! post=$post, get=$get, ".date("Y-m-d H:i:s")."\r\n");
    exit('success');
}
...
$sql=$sql." VALUES (171,$accountId,'$PayName','$serverId','$payMoney','$orderId','$dwFenBaoID','$Add_Time','1','$gameId','$clientType', '2')";
if (mysqli_query($conn,$sql) == False){
    write_log(ROOT_PATH."log","haima_callback_ios_error_", $sql.", post=$post, get=$get, ".mysqli_error($conn)."  ".date("Y-m-d H:i:s")."\r\n");
    exit('fail');
}
$isPay = 1;
if($trade_status == '1'){
...
function ChangPayLog($OrderID,$rpCode,$PayMoney){
    ...
    if (mysqli_query($conn,$sql) == False){
        write_log(ROOT_PATH."log","haima_callback_ios_error_", "sql=$sql".date("Y-m-d H:i:s")."\r\n");
    }
}