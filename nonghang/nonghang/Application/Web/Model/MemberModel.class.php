...
class MemberModel extends Model {
    ...
    function getUser($map){
    	...
    	$map['cinemaGroupId']=$wwwInfo['cinemaGroupId'];
    ...
    function loginMember($member,$cinemaCode,$passWord){
    	$wwwInfo = session('wwwInfo');
    	$type=M('cinemaMemberType')->where(array('memberType'=>$member['LevelCode'],'cinemaGroupId'=>$wwwInfo['cinemaGroupId'], 'cinemaCode'=>$cinemaCode))->find();
    	if(empty($type['memberGroupId'])){
    		$data['status']=1;
    		$data['info']='该会员卡用户禁止使用';
    	}else{
    		$user=M('member')->where('(cardNum='.$member['CardNum'].' or mobileNum='.$member['CardNum'].') and businessCode='.$cinemaCode.' and cinemaGroupId = ' . $wwwInfo['cinemaGroupId'])->find();
    		...
    		if(empty($user)){
    			$userarr['cinemaGroupId'] = $wwwInfo['cinemaGroupId'];
    			$userarr['otherName'] = $member['CardNum'];
