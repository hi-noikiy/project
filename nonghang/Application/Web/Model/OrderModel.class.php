...
class OrderModel extends Model {
	...
	function getList($field='',$map=array(),$start=0,$limit=9999999,$order='lockTime desc'){
		$map['visible']=0;
		 ...
		 foreach ($orders as $k=>$value) {
		 	$orders[$k]['orderTime']=date('Y-m-d H:i:s',$value['orderTime']);
		 	$orders[$k]['startTime']=date('Y-m-d H:i',$value['startTime']);
	...
	function addObj($lock,$plan,$user,$str,$mobile,$seatInfo,$fead){
        ...
        $arr['cinemaGroupId']=$wwwInfo['cinemaGroupId']; //购票系统
    	if(M('orderFilm')->add($arr)){
    		return $lock['OrderCode'];
    	}else{
    		return 0;
    	}
    }
    ...
    public function updateOrder($uid){
    	$myOrderFilm=M('OrderFilm')->where('uid='.$uid.' and status=0 and way="www"')->find();
    	$t=0;
