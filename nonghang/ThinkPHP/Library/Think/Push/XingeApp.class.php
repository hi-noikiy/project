...
class XingeApp {
	...
	{
		$push = new \Think\Push\XingeApp($accessId, $secretKey);
		$mess = new \Think\Push\Message();
		$mess->setTitle($title);
		$mess->setContent($content);
		$mess->setType(\Think\Push\Message::TYPE_NOTIFICATION);
		$mess->setStyle(new \Think\PushStyle(0, 1, 1, 1, 0));
	...
	{
		$push = new \Think\Push\XingeApp($accessId, $secretKey);
		$mess = new \Think\Push\MessageIOS();
	...
	public static function PushAccountAndroid($accessId, $secretKey, $title, $content, $account)
	{
		...
		$mess->setContent($content);
		$mess->setType(\Think\Push\Message::TYPE_NOTIFICATION);
		$mess->setStyle(new \Think\Push\Style(0, 1, 1, 1, 0));
		$action = new \Think\Push\ClickAction();
		$action->setActionType(\Think\Push\ClickAction::TYPE_ACTIVITY);
		$mess->setAction($action);
	...
	public function  PushSingleDevice($deviceToken, $message, $environment=0)
	{
		...
		if (!($message instanceof \Think\Push\Message) && !($message instanceof \Think\Push\MessageIOS)) return $ret;
		...
		if($message instanceof \Think\Push\Message) $params['multi_pkg'] = $message->getMultiPkg();
	...
	public function  PushSingleAccount($deviceType, $account, $message, $environment=0)
	{
		...
		if (!($message instanceof \Think\Push\Message) && !($message instanceof \Think\Push\MessageIOS))
		{
			$ret['err_msg'] = 'message is not android or ios';
			return $ret;
		}
		if($message instanceof \Think\Push\MessageIOS)
		{
			if($environment!=\Think\Push\XingeApp::IOSENV_DEV && $environment!=\Think\Push\XingeApp::IOSENV_PROD)
			{
		...
		$params['expire_time'] = $message->getExpireTime();
		$params['send_time'] = $message->getSendTime();
		if($message instanceof \Think\Push\Message)
	...
	{
		$paramsBase = new \Think\Push\ParamsBase($params);
		$sign = $paramsBase->generateSign(\Think\Push\RequestBase::METHOD_POST, $url,  $this->secretKey);
		$params['sign'] = $sign;
		
		$requestBase = new \Think\Push\RequestBase();
		$ret = $this->json2Array($requestBase->exec($url, $params, \Think\Push\RequestBase::METHOD_POST));
	...
	public function BatchSetTag($tagTokenPairs) 
	{
		...
		foreach ($tagTokenPairs as $pair) {
			if (!($pair instanceof \Think\Push\TagTokenPair)) {
