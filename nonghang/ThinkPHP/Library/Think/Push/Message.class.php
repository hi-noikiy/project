...
class Message {
	...
	public function __construct()
	{
		...
		$this->m_multiPkg = 0;
		$this->m_raw = "";
		$this->m_style = new \Think\Push\Style(0);
		$this->m_action = new \Think\Push\ClickAction();
	}
	...
	{	
		...
		if($this->m_type == self::TYPE_NOTIFICATION)
		{
			if(!($this->m_style instanceof \Think\Push\Style) || !($this->m_action instanceof \Think\Push\ClickAction))
				return false;
		...
		{
			if(!($value instanceof \Think\Push\TimeInterval) || !$value->isValid())
				return false;
		}
