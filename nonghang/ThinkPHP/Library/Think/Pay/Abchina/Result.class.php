...
class Result extends \Think\Pay\Abchina\core\TrxException {
	...
	public function init($aMessage) {
		$this->iLogWriter = new \Think\Pay\Abchina\core\LogWriter();
		try {
			...
			$this->MerchantConfig->verifySignXML($tMessage);
			$this->iLogWriter->logNewLine("验证通过！\n");
			$tResponse = new \Think\Pay\Abchina\core\TrxResponse($tMessage);
