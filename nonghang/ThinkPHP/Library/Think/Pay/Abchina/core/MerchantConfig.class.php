...
class MerchantConfig {
	...
	//商户号
	private static $iMerchantIDs = array ('103881310000016');
...
public static function getLogWriterObject($LogWriter) {
	...
	private static function bundle() {
		if (!self :: $iIsInitialed) {
			...
			// //2、读取系统配置段
			self :: $iTrustPayConnectMethod = self :: $config['TrustPayConnectMethod'];
			self :: $iTrustPayServerName = self :: $config['TrustPayServerName'];
			self :: $iTrustPayServerPort = intval(self :: $config['TrustPayServerPort']);
			...
			self :: $iTrustPayTrxURL = self :: $config['TrustPayTrxURL'];
			self :: $iTrustPayIETrxURL = self :: $config['TrustPayIETrxURL'];
			self :: $iMerchantErrorURL = self :: $config['MerchantErrorURL'];
			$tNewLine = self :: $config['TrustPayNewLine'];
			if ($tNewLine == '1') {
				self :: $iNewLine = '\n';
			} else {
				self :: $iNewLine = '\r\n';
			}
			$tTrustPayCertFile = self :: $config['TrustPayCertFile'];
			...
			self :: $iIsLog = 0;
			if (self :: $iIsLog) {
				self :: $iLogPath = self :: $config['LogPath'];
			}

			//3、读取商户号
			self :: $iMerchantIDs = array_filter(array_map('trim', explode(',', self :: $config['MerchantID'], 100)));
			self :: $iMerchantNum = count(self :: $iMerchantIDs);

			//4、读取商户证书
			self :: $iKeyStoreType = self :: $config['MerchantKeyStoreType'];
	...
	private static function bindMerchantCertificateByFile() {
		$tMerchantCertFiles = self :: $config['MerchantCertFile'];
		$tMerchantCertPasswords = self :: $config['MerchantCertPassword'];
	...
	public static function verifySignXML($aMessage) {
		self :: bundle();
		$aMessage = new \Think\Pay\Abchina\core\XMLDocument($aMessage);

		$tTrxResponse = $aMessage->getValue('Message');
		if ($tTrxResponse == null) {
			throw new \Think\Pay\Abchina\core\TrxException(TrxException :: TRX_EXC_CODE_1301, TrxException :: TRX_EXC_MSG_1301, '无[Message]段！');
		}
		$tAlgorithm = $aMessage->getValue('Signature-Algorithm');
		if ($tAlgorithm == null) {
			throw new \Think\Pay\Abchina\core\TrxException(TrxException :: TRX_EXC_CODE_1301, TrxException :: TRX_EXC_MSG_1301, '无[Signature-Algorithm]段！');
		}
		$tSignBase64 = $aMessage->getValue('Signature');
		if ($tSignBase64 == null) {
			throw new \Think\Pay\Abchina\core\TrxException(TrxException :: TRX_EXC_CODE_1301, TrxException :: TRX_EXC_MSG_1301, '无[Signature]段！');
		}
		...
		if (openssl_verify($data, $tSign, $key, OPENSSL_ALGO_SHA1) == 1) {
			...
			throw new \Think\Pay\Abchina\core\TrxException(TrxException :: TRX_EXC_CODE_1302, TrxException :: TRX_EXC_MSG_1302);
		}
