...
class AlipayNotify {
	...
	function __construct($alipay_config){
		if($alipay_config['pay_way']=='www'){
			$this->alipay_gateway_new='https://mapi.alipay.com/gateway.do?';
		}elseif($alipay_config['pay_way']=='wap'){
		...
		$alipay_config['seller_email']=$alipay_config['sellerEmail'];
		$alipay_config['partner']=$alipay_config['partnerId'];
		$alipay_config['sign_type']=$alipay_config['signType'];
		$alipay_config['key']=$alipay_config['alipayKey'];
	...
	function verifyNotify(){
		if(empty($_POST)) {//判断POST来的数组是否为空
			...
			if ($this->alipay_config['sign_type'] == '0001') {
				$decrypt_post_para['notify_data'] = rsaDecrypt($decrypt_post_para['notify_data'], $this->alipay_config['privateKey']);
			}
	...
	function decrypt($prestr) {
		return rsaDecrypt($prestr, trim($this->alipay_config['privateKey']));
	}
	...
	function getSignVeryfy($para_temp, $sign, $isSort) {
		...
		switch (strtoupper(trim($this->alipay_config['sign_type']))) {
			case "MD5" :
				...
				break;
			case "RSA" :
				$isSgin = rsaVerify($prestr, trim($this->alipay_config['publicKey']), $sign);
				break;
			case "0001" :
				$isSgin = rsaVerify($prestr, trim($this->alipay_config['publicKey']), $sign);
				break;
