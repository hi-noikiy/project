...
function S($name,$value='',$options=null) {
    static $cache   =   '';
    if (!strstr($name, C('CACHE_NAME_LIST'))) {
        $name = C('CACHE_NAME_LIST') . $name;
    }
    ...
    if(''=== $value){ // 获取缓存
        ...
        $allCacheName = $cache->get(C('CACHE_NAME_LIST'));
        unset($allCacheName[$name]);
        $cache->set(C('CACHE_NAME_LIST'), $allCacheName);
        ...
        $allCacheName = $cache->get(C('CACHE_NAME_LIST'));
        $allCacheName[$name] = array('createTime' => date('Y-m-d H:i:s'), 'expiration' => $expire);

        $apiAllCacheNameExpire = strtotime(date('Y-m-d')) + 86400 - time();

        // print_r($allCacheName);

        $cache->set(C('CACHE_NAME_LIST'), $allCacheName, $apiAllCacheNameExpire);
        // echo $name . '<br />';
