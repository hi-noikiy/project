...
myApp.onPageInit('bind', function (page) {
	var myApp = new Framework7({
		modalTitle:'温馨提示',
		modalButtonOk: '确认',
    	modalButtonCancel: '取消'
	...
	$$('.loginBtn').on('click', function () {
		myApp.confirm('手机账户：12345678910已注册，余额￥500，积分200，绑定后余额和积分会被覆盖，是否继续绑定？', function () {
			myApp.alert('绑定成功');
		});
	});
...
myApp.onPageInit('buying', function (page) {
        ...
        $$('.item-right p a').on('click', function () {
            $('.shadow').show();
            var a = parseFloat($(this).parents('.item-right').find('b').text()).toFixed(2);//获取价格;
