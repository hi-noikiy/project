...
var layDiv;
var Connect = '?';
function lay(url,title,width,height)
{
    if (!width )width = 350;
	...
	url += Connect + 'time='+new Date().getTime();
	...
	layDiv = $.layer({
		...
		offset: [height+'px','50%'],
		area: [width+'px','auto'],
		border : [3, 1, '#4FA8DB', true],
		shade : [0.05 , '#000' , true],
		zIndex : 90,
		page: {html: content},
		success: function(){layer.close(loadi);}
	});
}
...
function myConfirm(msg,type,yes,no)
{
	if(type=='') type = 1;
	$.layer({
		shade: [0],
