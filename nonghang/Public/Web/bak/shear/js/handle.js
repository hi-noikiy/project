...
window.ShearPhoto.MINGGE(function() {
			 ...
			 //var wwwUrl= "http://localhost:8082/"; 
			 var wwwUrl= "http://wap.zmaxfilm.net:8181/";
	/*
	 ...
	 */
		...
		/*HTML5重点功能*/	 HTML5ZIP:[900,0.9],//HTML5截图前载入的大图 是否压缩图片(数组成员 是数字) ，如果不压缩的话填false，在处理特效时或者拉伸时会明显出卡顿,不流畅！官方强烈建意你进行设置 ，默认填写的是[900,0.9] ,代表宽和高都不能大于900，质量是0.9（最大是1） 
			   ...
			   /*记住 preview 尽量设false*/ 
					          ...
					          url:relativeUrl+"upload",   //后端处理地址，保证正确哦，这是常识，连这个地址都能写错，你就是菜B，已经在本版本中帮你加入相对路径，你基本不用改这里了
			        
					    scopeWidth:320,                 //可拖动范围宽  也就是"main"对象的初始大小(整数型，禁止含小数点) 宽和高的值最好能一致
		...
		Shear.complete=function(serverdata) {//截图成功完成时，由shearphoto.php返回数据过来的成功包
					...
					creatImg.innerHTML='<strong><i></i>恭喜你！截图成功</strong> <p>以上是你图片的' + 1 + '种尺寸</p><a href="javascript: parent.window.location.reload() ;" id="completeA33">完成</a>';
					complete.appendChild(creatImg);
...
var up = new ShearPhoto.frameUpImg({
	         ...
	         url:relativeUrl+"upload",            //HTML5切图时，不会用到该文件，后端处理地址，保证正确哦，这是常识，连这个地址都能写错，你就是菜B，已经在本版本中帮你加入相对路径，你基本不用改这里了
...
webcam.set_api_url(relativeUrl+"upload"); //拍照连接后端的URL文件，后端处理地址，保证正确哦，这是常识，连这个地址都能写错，你就是菜B，已经在本版本中帮你加入相对路径，你基本不用改这里了    
