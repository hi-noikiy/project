...
window.onload = function(){
	...
	var i = 0;
	oUl.style.width = oLi.length * (oLi[0].offsetWidth + 14)+'px';
...
$(function(){
	...
	$('.preDisabled,.next').show();
	$('.nextDisabled,.pre').hide();
	$('#conter li').click(function(){
		$(this).siblings().removeClass('on');
		$(this).addClass('on');
		$('.bigpic .btn').show();
		var qsrc=$('.on').find('img').attr('src');
		var firstSrc=$('.album li').first().find('img').attr('src');
		var lastSrc=$('.album li').last().find('img').attr('src');
		if(qsrc==firstSrc){
			$('.pre').hide();
			$('.preDisabled').show();
		}else if(qsrc==lastSrc){
			$('.next').hide();
			$('.nextDisabled').show();
		}
	...
	//});
	$('.pre').click(function(){
		$(this).parents('.album').find('.on').removeClass('on').prev().addClass('on');
		$('.next').show();
		var qsrc=$('.on').find('img').attr('src');
		//alert(qsrc);
		var firstSrc=$('.album li').first().find('img').attr('src');
	...
	$('.next').click(function(){
			$(this).parents('.album').find('.on').removeClass('on').next().addClass('on');
			var qsrc=$('.on').find('img').attr('src');
			var lastSrc=$('.album li').last().find('img').attr('src');
			//alert(lastSrc);
			$('.pre').show();
