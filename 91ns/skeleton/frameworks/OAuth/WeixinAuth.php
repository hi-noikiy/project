...
class WeixinAuth extends OAuthBase
{
    ...
    public function __construct() 
    {
        ...
        $this->clientId = $config->oauth->weixin->appId;
        $this->clientSecret = $config->oauth->weixin->appKey;

        $request = $di->get('request');
        $serverName = $request->getServerName();
       // $serverName = 'www.91ns.com';
    ...
    {        
        ...
        $params['state'] = $state; 
//         print_r($this->authorizeURL . "?" . http_build_query($params) . '#wechat_redirect');
//         die();
