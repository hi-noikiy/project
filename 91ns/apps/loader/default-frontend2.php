...
$di->set('volt', function($view, $di) use ($config) {
    ...
    $compiler->addFilter('ns_birthday_md', function($resolvedArgs, $exprArgs) {
        return 'date("m月d日",' . $resolvedArgs . ')';
    });
	...
	$compiler->addFilter('ns_css', function($resolvedArgs, $exprArgs) {
		$css = $this->config->application->assetsDebug?'web/css2/':$this->config->url->cssURL;;
		$resolvedArgs = substr($resolvedArgs, 1, -1);
        return '"'.$css.$resolvedArgs.'.css"';
    });

	$compiler->addFilter('ns_js', function($resolvedArgs, $exprArgs) {
		$js = $this->config->application->assetsDebug?'web/js2/':$this->config->url->jsURL;
		$resolvedArgs = substr($resolvedArgs, 1, -1);
        return '"'.$js.$resolvedArgs.'.js"';
    });
