...
class UserFamilyController extends UserController
{
    ...
    public function indexAction(){
        //跳转到新版家族页
        return $this->redirect('personal/imanchor?secondpage=myfamil');
    ...
    public function getFamilyListAction(){
        if($this->request->isPost()){
            $orderType = $this->request->getPost('orderType');
            $page = $this->request->getPost('page');
            $pageSize = $this->request->getPost('pageSize');

            $result = $this->familyMgr->getFamilyListNew($orderType, $page, $pageSize);
    ...
    public function getSigningInfoAction(){
     ...
     * 根据家族长uid，获得成员列表
     */
    public function getFamilyMemberListAction(){
        if($this->request->isPost()){
            $uid = $this->request->getPost('uid');
            $result = $this->familyMgr->getFamilyMemberByCreatorId($uid);
    ...
    public function getFamilyAnchorsAction(){
        if($this->request->isPost()){
            ...
            $page = $this->request->getPost('page');
            $pageSize = $this->request->getPost('pageSize');
            $result = $this->familyMgr->getAnchorsByFamilyId($familyId, $page, $pageSize);
            return $this->status->ajaxReturn($result['code'], $result['data']);
        }
     ...
     * 修改家族皮肤
     */
    public function updateFamilySkinAction(){
        if($this->request->isPost()){
//            $fid = $this->request->getPost('fid');
            $backgroundColor = $this->request->getPost('backgroundColor');
            $styleType = $this->request->getPost('styleType');
            $backgroundImg = $this->request->getPost('backgroundImg');
            $result = $this->familyMgr->updateFamilySkin($backgroundColor, $styleType, $backgroundImg);
            return $this->status->ajaxReturn($result['code'], $result['data']);
        }
     ...
     * 上传家族皮肤
     * */
    public function uploadFamilySkinAction(){
        if($this->request->isPost()){
            $fid = $this->request->getPost('fid');
            $result = $this->familyMgr->uploadFamilySkinPic($fid);
            if($result['code'] == $this->status->getCode('OK')){
                $revert = $this->status->ajaxGetReturnData($this->status->getCode('OK'), $result['data']);
            }else{
                $revert = $this->status->ajaxGetReturnData($result['code'], $result['data']);
        ...
        echo '<!DOCTYPE html><html><head><title></title></head><body></body><script type="text/javascript">window.parent.uploadImageCallback('.$revert.');</script>';exit;
    }
...
//    }
    ...
    public function getFamilyAnchorRankAction(){
        if($this->request->isPost()){
            $familyId = $this->request->getPost('familyId');
            $type = $this->request->getPost('type');
            $dtype = $this->request->getPost('dtype');
            $result = $this->familyMgr->getFamilyAnchorRank($familyId, $type, $dtype);
            return $this->status->ajaxReturn($result['code'], $result['data']);
        }
     ...
     * 获得家族logo
     */
    public function uploadFamilyLogoPicAction(){
        if($this->request->isPost()){
            $fid = $this->request->getPost('fid');
            $result = $this->familyMgr->uploadFamilyLogoPic($fid);
            return $this->status->ajaxReturn($result['code'], $result['data']);
        }
