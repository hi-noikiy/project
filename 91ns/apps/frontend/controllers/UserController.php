    ...
    */
    public function loginOfThirdCallbackAction(){
        ...
        if($code != NULL){
            ...
            if($result['code'] == $this->status->getCode('OK')){
                $result = $result['data'];
                return $this->redirect("thirdloginsuccess.html?state=/user/autoLogin&state2={$state}&status=0&first={$result['first']}&ns_source={$result['ns_source']}&type={$result['type']}");
            }

            $msg = $this->status->getCodeInfo($result['code']);
