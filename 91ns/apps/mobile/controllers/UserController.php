...
// * 修改密码
    ...
    */
    public function loginOfThirdAction(){
        ...
        //$url = "https://api.weibo.com/oauth2/authorize?response_type=code&client_id=3223364004&redirect_uri=http%3A%2F%2Fwww.91ns.com%2Fuser%2Floginofthirdcallback&state=http%3A%2F%2Fwww.91ns.com%2Flogin";

        //$url = 'http://www.91ns.net/static/douzilogin?type=1&state=http%3A%2F%2Fwww.91ns.net%2Flogin';
    ...
    */
    public function loginOfThirdCallbackAction(){
        ...
        if($code != NULL){
            ...
            if($result['code'] == $this->status->getCode('OK')){
                return $this->redirect("thirdloginsuccess.html?state=/user/autoLogin&state2={$state}&status=0");
            }
            return $this->redirect("thirdloginsuccess.html?state=/user/autoLogin&state2={$state}&status={$result['code']}&msg={$result['data']['info']}");
        }
