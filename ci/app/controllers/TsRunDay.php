...
class TsRunDay extends CI_Controller{
    ...
    public function send3nologin()
    {
    	...
    	$etime = strtotime(date('Ymd',strtotime('-7 day')));
    	$sql = "SELECT last_login_mac as mac,$date as logdate FROM `push_regid` where logdate<$lastday and role_create_time<$etime;";
    	$query = $dbsdk->query($sql);
    	if($query){
    		$data = $query->result_array();
    		if($data){
