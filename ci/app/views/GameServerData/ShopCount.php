    ...
    var dataOption = {
        ...
        callback: function (result) {
            if (result) {
                ...
                var table_html= '';
                var table_header = '<tr><th>项目<th/>';
                for (var i=0;i<result['header'].length;i++) {
                    table_header += "<th>"+result['header'][i]+"</th>";
                }
                table_header += '</tr>';
                for (var param in result['data']) {
                    if (!result['data'].hasOwnProperty(param)) continue;
                    for (var date in result['data'][param]) {
                        table_html += '<tr>' +
                            '<td>'+result['consume_types'][param]+'最高出售物品</td>'+
                            '</tr>';
                        table_html += '<tr>' +
                            '<td>'+result['consume_types'][param]+'最高物品购买次数</td>'+
                            '</tr>';
                        table_html += '<tr>' +
                            '<td>'+result['consume_types'][param]+'最高物品占比</td>'+
                            '</tr>';
                    }
                }

                $("#thead").html(table_header);
