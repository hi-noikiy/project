...
if(isset($code) && !empty($code)){
	...
	if(isset($result['id'])){
		write_log(ROOT_PATH."log","guanwang_token_phone_",json_encode($result).",have,post=$post, ".date("Y-m-d H:i:s")."\r\n");
		$insert_id = intval($result['id']);
	} else {
		write_log(ROOT_PATH."log","guanwang_token_phone_","no have,post=$post, ".date("Y-m-d H:i:s")."\r\n");
		...
		$isNew = 1;
		write_log(ROOT_PATH."log","guanwang_token_phone_",$sql_game.",post=$post, ".date("Y-m-d H:i:s")."\r\n");
	}
...
if(mysqli_query($conn,$sql)){
	...
	write_log(ROOT_PATH."log","guanwang_token_log_",json_encode($data).date("Y-m-d H:i:s")."\r\n");
