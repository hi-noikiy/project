...
if(isset($ret['ret']) && $ret['ret'] == 0){
	...
	if(false == $query=mysqli_query($conn,$sql)){
		write_log(ROOT_PATH."log","ysdk_login_error_"," sql error!, sql=$sql, ".date("Y-m-d H:i:s")."\r\n");
		exit('3 0');
	}
	...
	if(mysqli_query($conn,$sql_game) == false){
		write_log(ROOT_PATH."log","ysdk_login_error_"," sql error!, sql=$sql_game, ".date("Y-m-d H:i:s")."\r\n");
		exit('3 0');
	}
	...
	write_log(ROOT_PATH."log","ysdk_login_error_","curl error ! ret=$return, ".date("Y-m-d H:i:s")."\r\n");
	exit("4 0");
}
?>