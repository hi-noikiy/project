...
$get = serialize($_GET);
write_log(ROOT_PATH."log","meitu_callback_info_","post=$post,get=$get, ".date("Y-m-d H:i:s")."\r\n");
if(!in_array($_POST['order_status'], ['TRADE_FINISHED','TRADE_SUCCESS'])){ //支付失败
	write_log(ROOT_PATH."log","meitu_callback_error_","pay error，{$_POST['tradeStatus']}, post=$post,get=$get, ".date("Y-m-d H:i:s")."\r\n");
	exit('fail');
}
$sign = $_REQUEST["sign"];
...
if($sign != $data['sign']) {
    write_log(ROOT_PATH."log","meitu_callback_error_","sign error,{$data['sign']},{$key}, post=$post,get=$get, ".date("Y-m-d H:i:s")."\r\n");
    exit('FAILURE');
}
$conn = SetConn(88);
$orderId = $extendsInfo;
...
if(!$result){
    ...
    if(!$result_account['NAME']){
        write_log(ROOT_PATH."log","meitu_callback_error_", "account is not exist.  ".date("Y-m-d H:i:s")."\r\n");
        exit("FAILURE");
    }else{
    ...
    if (mysqli_query($conn,$sql) == False){
        write_log(ROOT_PATH."log","meitu_callback_error_","sql=$sql, ".date("Y-m-d H:i:s")."\r\n");
        exit('FAILURE');
    }
