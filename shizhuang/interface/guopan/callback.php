...
$playerId = $serialNumberArr[2];
$account_id = $serialNumberArr[3];
$channel = $serialNumberArr[4];
$isgoods = 0;
global $key_arr;
...
if($status == 1){
	...
	$sql_account = "select NAME,dwFenBaoID,clienttype from $acctable where id=$account_id limit 1;";
	...
	$sql=$sql." VALUES (115,$account_id,'$playerId','$money','$PayName','$server_id','$money','$trade_no','$dwFenBaoID','$Add_Time','1','$game_id','$clienttype',1,'$isgoods')";
	if (mysqli_query($conn,$sql) == False){
		write_log(ROOT_PATH."log","guopan_callback_error_", $sql." ".mysqli_error($conn)."  ".date("Y-m-d H:i:s")."\r\n");
		exit("fail");
	}
	WriteCard_money(1,$server_id, $money,$playerId, $trade_no,8,0,0,$isgoods);
	sendTongjiData($game_id,$account_id,$server_id,$dwFenBaoID,0,$money,$trade_no);
	exit("success");
