...
foreach ($orderInfo as $v){
    ...
    if($status==0 && $quantity>0 && $money){
        ...
        $rpCode = 1;
        $insert_sql = " insert into web_pay_log(CPID,ServerID,PayCode,PlayerID,PayMoney,PayName,dwFenBaoID,Add_Time,rpCode,PayID,OrderID,game_id) values('19',$server_id,'$currency','$player_id','$money','$PayName','$dwFenBaoID','$Add_Time','$rpCode','$account_id','$orderId','$game_id') ";
        if(mysqli_query($conn, $insert_sql)){
            $gameMoney = $yuanbao;//转化为RMB
            WriteCard_money(1,$server_id, $gameMoney,$player_id, $orderId,8,0,$type);//写入游戏库
            //统计数据
