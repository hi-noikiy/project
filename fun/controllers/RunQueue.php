...
//        if ($token != md5($this->auth_conf[$appid] . $data)) return false;
	...
	private function insert($table,$savedatas,$db='')
    {
    	if(!$db){
    	...
    	$msql = "insert into $table(".implode(',', array_keys($this->data)).") values(".implode(',', array_values($this->data)).") ON DUPLICATE KEY UPDATE ";
 ...
 public function UserInfo()
   ...
   */
    ...
    public  function  FirstRecharge(){  
        ...
        parent::log($this->db->last_query(), LOG_PATH . '/firstlog.log');
        parent::log(json_encode($this->db->last_query()),LOG_PATH . '/firstlog.log');        
     ...
     * @author 王涛 20170303
     */
    public function DevEmoney()
    {
    	if (!empty($this->data_multi)) {
    		foreach ($this->data_multi as &$v){
    			$this->data['logdate']   = date('Ymd',  $this->data['created_at']);
    		}
    	}else{
    		$this->data['logdate']   = date('Ymd',  $this->data['created_at']);
    	}
    	$this->save('dev_emoney');
    }
    /**
     * 任务用户参与
     *
    ...
    if (!empty($this->data_multi)) {
    		foreach ($this->data_multi as &$v){
    			$v['logdate']   = date('Ymd',  $v['tasktime']);
    			unset($v['appid']);
    		}
    	}else{
    		$this->data['logdate']   = date('Ymd',  $this->data['tasktime']);
    		unset($this->data['appid']);
    	}
    	$this->insert('u_task_user');
    }
    /**
     * 用户投票
     *
    ...
    public function Sharelog()
    {
    	if (!empty($this->data_multi)) {
    		foreach ($this->data_multi as &$v){
    			$v['logdate']   = date('Ymd',  $v['share_time']);
    			unset($v['appid'],$v['created_at']);
    		}
    	}else{
    		$this->data['logdate']   = date('Ymd',  $this->data['share_time']);
    		unset($this->data['appid'],$this->data['created_at']);
    	}
    	
    	$this->insert('u_share_log');
    }
    /**
     * 借衣日志
     *
