...
//            'wau_ac'=>'周活跃账号','mau_ac'=>'月活跃账号'
    ...
    public function travel(){
        ...
        if (parent::isAjax ()) {
            ...
            $register_date=$this->input->get ( 'reg1' ) ? $this->input->get ( 'reg1', true ) : '';
    
            $where ['date'] = date ( 'Ymd', strtotime ( $beginDate ) );
            $where ['enddate'] = date ( 'Ymd', strtotime ( $endDate ) );
            
            if($register_date){
                $where ['register_date'] = date ( 'Ymd', strtotime ( $register_date ) );
            }
           
            
            
            $group="b.param";
            $field="COUNT(DISTINCT b.accountid) cnt,param";
            $this->loadModel();
            $data = $this->player_analysis_model->travel($table, $where, $field, $group, $order, $limit);
            ...
            $this->data ['hide_end_time'] = true;
            $this->data ['register_start'] = true;
            $this->data ['type_list'] = $types;
            $this->data ['hide_server_list'] = true;
            $this->data ['hide_channel_list'] = true;
             
            $this->body = 'PlayerAnalysis/travel';
            $this->layout ();
        }
    }
