...
class AutoRunBak extends CI_Controller{
    ...
    public function run_index()
    {
    	$sql = '';
    	$count = 0;
    	$handle = fopen("/data/log/site/api/index_20171222.log", "r");
    	$i = 0;
    	if ($handle) {
    		while (!feof($handle)) {
    ...
    public function run_login()
    {
    	...
    	$sql = '';
    	$count = 0;
        $handle = fopen("/data/log/site/api/Login_20171222.log", "r");
        $i = 0;
        if ($handle) {
            while (!feof($handle)) {
    ...
    public function run_register()
    {
    	...
    	$sql = '';
    	$count = 0;
    	$handle = fopen("/data/log/site/api/Register_20171230.log", "r"); 
    	$i = 0;
    	if ($handle) {
    		while (!feof($handle)) {
    			$buffer = fgets($handle, 4096);
...
SQL;
    		...
    		public function run_device()
    		{
    			...
    			$sql = '';
    			$count = 0;
    			$handle = fopen("/data/log/site/api/DeviceActive_20171222.log", "r");    		                                
    			$i = 0;
    			if ($handle) {
    				while (!feof($handle)) {
    					$buffer = fgets($handle, 4096);
