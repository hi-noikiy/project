 ...
 $res_pre       foreach ($res['remain'] as $item) {
    ...
    public function getRegisterProcessData()
    {
       ...
       // if (!$typeid_list) $typeid_list = [0,24];
        if (!$typeid_list) $typeid_list = [1,2];
        $typeid_list = array_unique(array_merge([1,2], $typeid_list));
      ...
      // print_r($output);exit;
        ...
        foreach ($output as $k=>$item) {
            $data[$item['hour']][$item['type_id']] = array(
              ...
              //  'rate'  =>100*($item['cnt'] / $total[$item['hour']] )
            );
        }
    ...
    public function vip_wealth()
    {
    	if (parent::isAjax()) {
			...
			$sdk = $this->load->database('sdk',true);
			$field = 'select viplev,sum(surplus_diamond) ssurplus_diamond,sum(surplus_gold) surplus_gold,AVG(surplus_diamond) asurplus_diamond,AVG(surplus_gold) asurplus_gold,COUNT(accountid) cont 
from u_dayonline a,(SELECT max(id) id from u_dayonline where online_date >= '.$date1.' AND online_date <= '.$date2.' GROUP BY accountid) b 
where a.id=b.id GROUP BY viplev';
    		$query = $sdk->query($field);
    		if ($query) $output = $query->result_array();
    		if (!empty($output))
     ...
     * 各等级玩家剩余金币、钻石统计
     */
    public function level_wealth()
    {
    	if (parent::isAjax()) {
        	// 时间选择
        	$date1 = $this->input->get('reg1');
			$date2 = $this->input->get('reg2');
			
			$date1 = date('Ymd', strtotime($date1));
			$date2 = date('Ymd', strtotime($date2));
			
			$sdk = $this->load->database('sdk',true);
			$field1 = 'select lev,sum(surplus_diamond) ssurplus_diamond,sum(surplus_gold) surplus_gold,AVG(surplus_diamond) asurplus_diamond,
AVG(surplus_gold) asurplus_gold,COUNT(a.accountid) cont 
from u_dayonline a,
(SELECT max(id) id,accountid from u_dayonline GROUP BY accountid) b,
(SELECT accountid from u_register WHERE reg_date>='.$date1.' and reg_date<='.$date2.') c
where a.id=b.id and b.accountid=c.accountid GROUP BY lev';
			$query = $sdk->query($field1);
    		if ($query) $output = $query->result_array();
    		if (!empty($output)) echo json_encode(['status'=>'ok','data'=>$output]);
    		else echo json_encode(['status'=>'fail']);
        }
     ...
     * 等级分布
     */
    public function players_condition()
    {
    	if (parent::isAjax()) {
        	// 注册时间选择
			...
			$reg1 = date('Ymd', strtotime($reg1));
			$reg2 = date('Ymd', strtotime($reg2));
			
			// 最后登录时间
			$date1 = $this->input->get('date1').' 00:00:00';
			$date2 = $this->input->get('date2').' 24:00:00';
			
			$date1 = strtotime($date1);
			$date2 = strtotime($date2);
		
			$sdk = $this->load->database('sdk',true);
			$field2 = 'SELECT lev,COUNT(*) cont from u_register a,u_last_login b WHERE b.last_login_time >= '.$date1.' AND
 b.last_login_time <= '.$date2.' AND a.reg_date>='.$reg1.' and a.reg_date<='.$reg2.' and a.accountid=b.accountid GROUP BY lev';
			$query = $sdk->query($field2);
    		if ($query) $output = $query->result_array();
			...
			$this->data['hide_server_list'] = true; 
			$this->data['hide_channel_list'] = true; 
            $this->body = 'Home/players_condition';
            $this->layout();
        }
        //$this->data['data'] = $data;
