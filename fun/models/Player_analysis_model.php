...
SQL;
	...
	public function newUser	($appid,$date1, $date2, $serverid = null, $channel = null) {
	  ...
	  /*
	   * 邀请好友需求  chenyanbin 2017/11/14
	   */
	  public function inviteFriend($where, $field, $group){

			$sql = "select viplev, count(1) sum from u_invite where 1=1";
	...
	/*
	 * 邀请好友需求详细  chenyanbin 2017/11/14
	 */
	public function inviteFriendFrame( $where, $field, $group )
	{
		$sql = "select $field  from u_invite where";
		if ($where['begintime'] && $where['endtime']) {
			$sql .= "(logdate>={$where ['begintime']} and  logdate<={$where ['endtime']})";
		}
		if (isset($where['viplev'])) {
			$sql .= " AND viplev = {$where['viplev']}";
		}
		if($group){
			$sql .= " group by $group";
		}
...
//  典型玩家数据 zzl  20171025
   ...
   *
   */
  public function getActiveDataSday($table, $where, $field, $group, $order, $limit) {  
     $sql="select SUM(dau) AS dau,sday FROM sum_au where 1=1";
     if($where['serverids']){
           $sql .= " and serverid IN(" . implode ( ',', $where ['serverids'] ) . ")";
     }
     if($where['channels']){
     ...
     if($where['date1'] && $where['date2']){
         $sql .= " and (sday>={$where['date1']} and sday<={$where['date2']})";
     }    
     ...
     * 旅行任务 zzl 20171221
     */
    public function travel($table, $where, $field, $group, $order, $limit){
        ...
        $sql="select $field from u_behavior_{$where['date']} b INNER JOIN u_register r on b.accountid=r.accountid where act_id=12";
         
    
        
        if ($where ['register_date']) {
            $sql .= " and  r.reg_date={$where['register_date']}";
        }
    ...
    public function classDistribution($table, $where, $field, $group, $order, $limit){
        ...
        $sql="select $field from u_last_login l INNER JOIN u_register r on l.accountid=r.accountid where 1=1 ";
      ...
      if ($where ['login_date']) {
            $sql .= " and  DATE_FORMAT(FROM_UNIXTIME(l.last_login_time),'%Y%m%d')={$where['login_date']}";
        }
