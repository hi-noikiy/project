...
class Sdk_sum_model extends CI_Model
{
     ...
     * @author 王涛 20170915
     */
    public function recharge($where=array(),$field='*',$group='',$order='')
    {
    	/*$begin = strtotime($where['begindate']);
    	$end = strtotime($where['enddate'])+(24*60*60-1);
    	$sql = "select FROM_UNIXTIME(b.created_at,'%Y%m%d') as day,sum(b.money) as allmoney,
    			count(distinct b.accountid ) as countAccountid, count(*) as count
    			 FROM ad_register a,u_paylog b
    			 where a.accountid=b.accountid  AND b.is_pay=0 AND b.created_at BETWEEN $begin AND $end";*/
    	$begin = $where['begindate'];
    	$end = $where['enddate'];
    	/*$sql = "select logdate as day,sum(b.money) as allmoney,
    	count(distinct b.accountid ) as countAccountid, count(*) as count
    	FROM ad_register a,u_paylog b
    	where a.accountid=b.accountid  AND b.is_pay=0 AND a.logdate BETWEEN $begin AND $end";
    	if($where['channels']){
    		$sql .= " AND a.media_source='{$where['channels']}'";
    	}
    	$sql .=" GROUP BY day";*/
    	if($begin>$end){
    		return array();
    	}
    	$sql = "select $begin as day,sum(b.money) as allmoney,
    	count(distinct b.accountid ) as countAccountid, count(*) as count
    	FROM ad_register a,u_paylog b
    	where a.accountid=b.accountid  AND b.is_pay=0 AND a.logdate =$begin and FROM_UNIXTIME(b.created_at,'%Y%m%d')= $begin";
    	$i=0;
    	while($begin<$end){
    		$begin = date('Ymd',strtotime("$begin +1 day"));
    		$sql .= " union select $begin as day,sum(b.money) as allmoney,
    	count(distinct b.accountid ) as countAccountid, count(*) as count
    	FROM ad_register a,u_paylog b
    	where a.accountid=b.accountid  AND b.is_pay=0 AND a.logdate =$begin and FROM_UNIXTIME(b.created_at,'%Y%m%d')= $begin";
    	}
