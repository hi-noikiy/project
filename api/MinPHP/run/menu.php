...
<?php } else{
    $sql = "select * from api where aid = '{$_GET['tag']}' and isdel='0' order by num asc, ord desc,id desc";
    $list = select($sql);?>
    <div class="form-group">
    ...
    <div class="list">
        <ul class="list-unstyled" style="padding:10px">
            <?php foreach($list as $v){ ?>
            <li class="menu" id="api_<?php echo md5($v['id']);?>" <?php $num=explode('.', $v['num']);?> <?php if(strpos($v['num'],'.')){ ?>style='display:none' pid="<?php echo $num[0];?>" <?php }else{ ?>onclick="show(<?php echo $num[0];?>)"<?php } ?>>
...
<script>
    ...
    function show(num){
        if($('li[pid="'+num+'"]').css('display')=='none'){
        	$('li[pid="'+num+'"]').show();
        }else{
        	$('li[pid="'+num+'"]').hide();
        }
