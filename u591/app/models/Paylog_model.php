...
SQL;
	...
	public function getData($serverId=0, $channelId=0) {
		$sql    = <<<SQL
SELECT DATE_FORMAT(FROM_UNIXTIME(`created_at`),'%Y-%m-%d') as day,DATE_FORMAT(FROM_UNIXTIME(`created_at`),'%Y%m%d') as logdate, sum(money) as allmoney,count(distinct accountid ) as countAccountid, count(*) as count FROM u_paylog
WHERE appid=? AND created_at BETWEEN ? AND ?
SQL;
		$sql .= " AND is_pay=0";
		if (is_numeric($serverId) && $serverId>0) 
