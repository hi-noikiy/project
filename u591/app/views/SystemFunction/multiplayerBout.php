...
$('#btype').change(function(){
    ...
    var dataOption = {
        title:'',
        autoload: false,     
      request_url:'<?php echo site_url('SystemFunctionNew/multiplayerBout');?>',
        callback: function (result) {
            if (result) {
                ...
                for(var i in result['data']){
                    if(!isNaN(i)){
                    	  if(typeof(result['data'][i]['turn_num'])=='undefined'){result['data'][i]['turn_num']=0;}
                    	  if(typeof(result['data'][i]['total'])=='undefined'){result['data'][i]['total']=0;}
                              
                    	table_html += '<tr>' +
                        '<td>'+result['data'][i]['turn_num']+'</td>' +
