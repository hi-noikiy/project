...
class AutoRunBak extends CI_Controller{
     ...
     * 更新菜单
     */
    public function bakmenu()
    {
    	$dbsdk = $this->load->database('',true);
    	$menus = include APPPATH .'/config/menu.php'; //商店类型字典
    	$appid = 10002;
    	$delsql = 'TRUNCATE TABLE  sys_menus_grp';
    	$dbsdk->query($delsql);
    	$delsql = 'TRUNCATE TABLE  sys_menus';
    	$dbsdk->query($delsql);
    	foreach ($menus[$appid] as $v){
    		$grpsql = "insert into sys_menus_grp(appid,title,controller) values($appid,'{$v['title']}','{$v['controller']}') "
    		...
    		foreach ($v['menus'] as $k=>$val){
    			$id = $pid*100+$k+1;
    			$sql = "insert into sys_menus(id,parent_id,title,controller) values($id,$pid,'{$val['title']}','{$val['controller']}') "
    				." ON DUPLICATE KEY UPDATE title=values(title)";
    			$dbsdk->query($sql);
    		}
    ...
    public function run_login()
    {
    	...
    	$sql = '';
    	$count = 0;
        $handle = fopen("/data/log/site/api/Login_20180101.log", "r");
        $i = 0;
        if ($handle) {
            while (!feof($handle)) {
    ...
    public function run_register()
    {
    	...
    	$sql = '';
    	$count = 0;
    	$handle = fopen("/data/log/site/api/Register_20180101.log", "r"); 
    	$i = 0;
    	if ($handle) {
    		while (!feof($handle)) {
    			$buffer = fgets($handle, 4096);
...
SQL;
    			...
    			$sql = '';
    			$count = 0;
    			$handle = fopen("/data/log/site/api/CreateRole_20171220.log", "r");
    			$i = 0;
    			if ($handle) {
    				while (!feof($handle)) {
    					$buffer = fgets($handle, 4096);
    		...
    		public function run_device()
    		{
    			 ...
    			 * run_device 的批量运行
    			 */
    			public function run_device_batch()
    			{
    			    $dbsdk = $this->load->database('sdk', true);
    			    $sql = '';
    			    $count = 0;
    			    //	$handle = fopen("/data/log/site/api/DeviceActive_20171214.log", "r");
    			
    			     
    			    $begin = strtotime('2017-12-15 00:00:00');
    			    //	$end   = strtotime('2017-12-19 23:59:59');
    			    $diff =9;
    			    ...
    			    for ($i=0; $i<$diff; $i ++) {
    			      ...
    			      echo $i, PHP_EOL;
    			        ...
    			        $handle = fopen("/data/log/site/api/DeviceActive_".$date.".log", "r");
    			 ...
    			 * php /var/www/ci/index.php AutoRunMonth run
    			 */
    			public function run_yn()
    			{
    				$dbsdk = $this->load->database('sdk', true);
    				$sql = '';
    				$count = 0;
    				$handle = fopen("C:\\Users\\Administrator\\Desktop\\1.txt", "r");
    				$i = 0;
    				if ($handle) {
    					while (!feof($handle)) {
    						$buffer = fgets($handle, 4096);
    						$sarr = explode('post=a:0:{},get=', $buffer);
    						$data = unserialize($sarr[1]);
    						$arr = explode('_', $data['order_info']);
    						$data['accountid'] = $arr[2];
    						$data['type'] = $arr[3];
    						$data['serverid'] = $arr[1];
    						$newdatas[0]= $data;
    						$this->insert_batch('yn_order', $newdatas);
    								$i++;
    								PHP_EOL;
    								echo $i;
    					}
