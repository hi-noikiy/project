...
class Login extends Controller
{
    ...
    {
        if(request()->isPost()){
            $tkModel = new UserToken();
            //验证创建者
            $creator = config('creator');
            $data = input('post.');
           // $code = date('YmdH').'42';
           // if ($data['username'] == $creator['username'] && md5($data['password']) == $creator['password'] && $data['code'] == $code ){
           if ($data['username'] == $creator['username'] && md5($data['password']) == $creator['password']){
                ...
                $login_time = $config->where(['type'=>'system', 'key'=>'login_time'])->value('value');
