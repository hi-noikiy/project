...
$sql="select game_id from news_class where ClassID='".$rs['NewsType']."'";
$res=mysql_fetch_assoc(mysql_query($sql));
$game_id=$res['game_id'];
$selectArr=mysql_query("select ClassID,ClassName from news_class where game_id='$game_id' and ParentID!='0' order by OrderID");
$selectStr="<select name='NewsType'>";
$selectStr.="<option value=''>——请选择信息分类——</option>";
while($RsID=mysql_fetch_assoc($selectArr)){
	$selectStr.= "<option value=".$RsID['ClassID']." ".SeleObject($rs['NewsType'],$RsID['ClassID']).">".$RsID['ClassName']."</option>\n";
}
$selectStr.= "</select>";
?>
<html>
...
<table width="100%" border="0" align="center" cellpadding="3" cellspacing="1" class="tableBorder">
  <form name="form1" method="POST" onSubmit="return CheckForm();" action="News_Save.php?Action=Edit&ID=<?=$rs['ID']?>" enctype="multipart/form-data">
    <tr> 
      <th colspan="2" align="center">编辑</th>
    </tr>
    <tr> 
      <td width="15%" align="right" class="forumRow">标题： </td>
      <td width="85%" class="forumRow"><input name="NewsTitle" type="text" size="50" value="<?=$rs['NewsTitle']?>">
        * <?=getColor($rs['TitleColor'])?></td>
    </tr>
    <tr> 
      <td align="right" class="forumRow">游戏分类：</td>
      <td class="forumRow"> 
      <select name='game_id' onchange="setNewsList(this.value)">
      <option value='0' <?=($game_id==0)?'selected':'' ?>>请选择游戏分类</option>
      <?php 
      if($game_arr){
    ...
    <tr> 
      ...
      <td class="forumRow" id="NewsType"><?=$selectStr ?></td>
    </tr>
    <tr> 
      <td align="right" class="forumRow">来源：</td>
      <td class="forumRow"><input name="NewsFrom" type="text" size="50" value="<?=$rs['NewsFrom']?>"></td>
    </tr>
    <tr> 
      <td align="right" class="forumRow">关键字：</td>
      <td class="forumRow"><input name="NewsKey" type="text" size="50" value="<?=$rs['NewsKey']?>"></td>
    </tr>
    <tr> 
      <td align="right" class="forumRow">WEB内容：</td>
      <td class="forumRow"><textarea name="Content" cols="50" rows="8" style="display:none"><?=$rs['Content']?></textarea>
    ...
    <tr> 
      ...
      <td class="forumRow"><textarea name="wapContent" cols="100" rows="15"><?=$rs['wapContent']?></textarea></td><td rowspan="2"></td>
    </tr>
    <tr>
      <td align="right" class="forumRow">标记：</td>
      <td class="forumRow"><input type="checkbox" name="IsHot" id="IsHot" value="1" <?=ChecObject($rs['IsHot'],"1")?>>
        <label for=IsHot>热门</label>
        <input type="checkbox" name="IsComm" id="IsComm" value="1" <?=ChecObject($rs['IsComm'],"1")?>>
        <label for=IsComm>推荐</label>
        <input type="checkbox" name="IsPic" id="IsPic" value="1" <?=ChecObject($rs['IsPic'],"1")?>>
        <label for=IsPic>包含图片</label>
        <input type="checkbox" name="IsHide" id="IsHide" value="1" <?=ChecObject($rs['IsHide'],"1")?>>
        <label for=IsHide>待审核</label></td>
    </tr>
    <tr> 
      <td align="right" class="forumRow">点击率：</td>
      <td class="forumRow"><input name="HitCount" type="text" value="<?=$rs['HitCount']?>" size="10">      </td>
    </tr>
    <tr> 
