...
if ($Action=="Order"){
	...
	$N=count($news_list);
	for ($I=0; $I<=$N-2; $I++) {
		$sql="Update news_class Set OrderID=$I Where ClassID=$news_list[$I]";
		mysql_query($sql);
	}
	header("Location: ?ClassID=$ParentID");
}
?>
<html>
<head>
...
if ($ParentID!=""){
	$sqlstr=" And ParentID=$ParentID Order By OrderID";
}else{
	$sqlstr=" And ParentID=0 Order By OrderID";
}
$sql="select ClassID,ClassName from news_class where 1=1".$sqlstr;
	...
	<input type="hidden" name="ParentID" value="<?=$ParentID?>">
      ...
      <td class="forumRow"> 
        <table width="100%" border="0" cellspacing="0" cellpadding="0" class="box">
          ...
          <tr > 
            <td height="40"> <input type="button" name="button_34" value="恢  复" class="box" style="WIDTH: 70px;HEIGHT: 22px;BACKGROUND-COLOR: #ffffff" onClick="javascript:location.href='?ClassID=<?=$ParentID?>';"> 
            </td>
          </tr>
          <tr > 
          ...
          <tr >
            <td height="40"><input type="button" name="back" value="返  回" class="box" style="WIDTH: 70px;HEIGHT: 22px;BACKGROUND-COLOR: #ffffff" onClick="window.location='Class_List.php?ClassID=<?=$ParentID?>'"></td>
          </tr>
        </table>
      </td>
		</tr>
