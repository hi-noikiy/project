...
if ($CPID != ""){
    ...
    $sql=$sql." And CPID in(3,6,7,10,11)";
}
if ($PayCode != ""){
...
if ($s_money != ""){
	...
	$PURL=$PURL."$s_emoney=$s_emoney&";
}
if ($e_money != ""){
    $sql=$sql." And PayMoney<='".intval($e_money)."'";
	$PURL=$PURL."$e_emoney=$e_emoney&";
}
...
<option value="10" <?php if($CPID==10){echo 'selected="selected"';} ?> >易宝</option>
<option value="3"<?php if($CPID==3){echo 'selected="selected"';} ?>>神州付</option>
<option value="6"<?php if($CPID==6){echo 'selected="selected"';} ?>>当乐</option>
<option value="7"<?php if($CPID==7){echo 'selected="selected"';} ?>>当乐189hi</option>
<option value="11"<?php if($CPID==11){echo 'selected="selected"';} ?>>9游</option>
</select>
	</td>
    ...
    <tr>
      ...
      <td class="forumRow">定单号显示灰色状态：写入游戏数据库失败</td>
    </tr>
  </form>
</table>
<?
...
?>
  <tr>
    ...
    <td nowrap class="forumRow"><?=$CPList[$rs["CPID"]]?>
    <?php
      if($rs["CPID"]==10){
            echo "易宝";
           }else if($rs["CPID"]==3){
    ...
    <td class="forumRow"><?if ($rs["IsUC"]==1){
		echo "<font color=#666666>".$rs["OrderID"]."</font>";
	}else{
		echo $rs["OrderID"];
	}?></td>
