...
if($orderid)
{
    $sql = "SELECT PayID,PayName,PayMoney,OrderID,Add_Time FROM pay_log where tag='0' and (rpCode='1' or rpCode='10') and OrderID='".$orderid."' limit 0,1";
    $res=mysql_query($sql);
    if($info = mysql_fetch_array($res))
    {
        ...
        if($addtag != '1')
        {
            $upsql = "update pay_log set tag='1' where OrderID='".$orderid."'";
            mysql_query($upsql);
        }
        else
        {
            $rets = updatePoints($info['PayID'],$info['PayMoney'],'f_dx',$info['OrderID'],'admin');
            if($rets!='ok')
            {
                if($rets=='okadd_err')
            ...
            {
                updateRankUp($info['PayID'],'f_dx',"admin");
            }
        }
    }
    else
...
SetConn(88);
$sql1 = "SELECT id,PayID,PayName,PayMoney,OrderID,Add_Time FROM pay_log where tag='0' and (rpCode='1' or rpCode='10')";
//echo $sql;
...
<table width="100%" border="0" align="center" cellpadding="3" cellspacing="1" class="tableBorder">
  <form name="SearchForm" method="POST" action="<?=getPath()?>">
    <tr>
      <th height="22" colspan="2" align="center">补积分</th>
    </tr>
    <tr>
...
<table width="100%" border="0" align="center" cellpadding="2" cellspacing="1" class="tableBorder">
  ...
  <tr>
    ...
    <td nowrap class="forumRow"><?=$rs['OrderID']?></td>
