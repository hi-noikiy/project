...
if(!$rs) {
    write_log(ROOT_PATH."log","yijie_callback_error_"," error=账号不存在, ".date("Y-m-d H:i:s")."\r\n");
    exit("success");
}else{
    $PayName=$rs["NAME"];
...
SetConn(88);
    ...
    if($result_count['rpCode']==1){
        write_log(ROOT_PATH."log","yijie_callback_error_log_"," 订单已存在 order_id=$order_id,".date("Y-m-d H:i:s")."\r\n");
        exit("success");//订单已存在
    }
...
if($money){
    ...
    $rpCode = 1;
    $insert_sql = " insert into pay_log(dwFenBaoID,CPID,ServerID,PayMoney,PayName,Add_Time,rpCode,PayID,OrderID,game_id) values('$dwFenBaoID','75','$server_id','$money','$PayName','$Add_Time','$rpCode','$account_id','$order_id','$game_id') ";
    if(mysql_query($insert_sql)){
    	try{
    		...
    		// $e->getMessage();
    		write_log(ROOT_PATH."log","yijie_callback_error_",$e->getMessage()." ,error=系统异常, ".date("Y-m-d H:i:s")."\r\n");
    	}
    }else{
        write_log(ROOT_PATH."log","yijie_callback_error_"," error=数据库错误,insert_sql=$insert_sql, ".date("Y-m-d H:i:s")."\r\n");
    }
}
write_log(ROOT_PATH."log","yijie_callback_error_","money:$money, error=数据异常，需查看日志, ".date("Y-m-d H:i:s")."\r\n");
exit("fail");
