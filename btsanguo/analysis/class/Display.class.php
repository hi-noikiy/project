...
//        echo $sql_total_emoney;
    ...
    public function ShowMarketPayDetail($stype, $group_by_day=false)
    {
//        return false;
        $where = ' WHERE 1=1';
        if ($this->bt==$this->et) {
        ...
        $where .= " AND stype=" . intval($stype);
        $sql = <<<SQL
  SELECT SUM(sum_emoney) AS sum_emoney, stype,serverid
  FROM `sum_behavior_pay` $where group by serverid order by serverid ASC
SQL;
        if ($group_by_day===true) {
            $sql = <<<SQL
  SELECT SUM(sum_emoney) AS sum_emoney, stype,sday
  FROM `sum_behavior_pay` $where group by sday order by sday ASC
SQL;
        }
