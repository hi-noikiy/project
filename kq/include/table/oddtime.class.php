...
class oddtime extends getList {
	...
	public function add($array) {
		...
		$sql="select count(*) as count from _web_hugh_pass where uid='{$array['uid']}' and hughdate='{$array ['supdate']}' limit 1;";
		...
		if($hughPassRows['count'])
			$hughPassFlag=true;
		// 判断是否超过7个工作日
		$supdate=$array['supdate'];
		$workDayList=$this->workDay();
		if(is_array($workDayList) && !in_array($supdate, $workDayList) && $hughPassFlag==false){
			echo "<script>alert('指纹异常申请日期不能超过7个工作日或非工作日不必填写指纹异常')</script>";
			go ( 'index.php?type=web&do=info&cn=oddtime' );
			exit ();
		}
