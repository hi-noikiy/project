...
class hugh extends getList {
	...
	public function add($array) {
		...
		if(is_array($workDayList) && !in_array($addDay, $workDayList)){	
		...
		*/
		...
		//从2015年6月1号执行
		$memberUserArr=array(331,362,381,331,259,116,316,394,307);
		
		if($preWorkDay && strtotime($array ['fromTime'])>=strtotime('2015-06-01')&& (!in_array($array['uid'], $memberUserArr) && $fromTime!='2015-06-16')){
	...
	protected function getOverTime($uid, $totalTime, $reserve, $fromTime){
		$overtime=$hughtime=0;
		//统计今天之前有多少加班时间
		$sql="select sum(totalTime) as overtime from _web_overtime where depTag='2' and perTag='2' and manTag='2' and uid=$uid and addDate<='$fromTime'";
