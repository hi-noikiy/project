...
//申请单作废函数voidFun
function voidFun(tab,id,fun){
	if(confirm('你确定要作废吗？')){
		$.get('command.php?action=voidForm&type='+tab+'&id='+id,function (data){
				window.location.href=window.location.href;
		});
	}
}
...
//审批撤销函数voidCancle
function voidCancle(tab,id,tag,fun){
	if(confirm('你确定要撤销吗？')){
            $.get('command.php?action=voidCancle&type='+tab+'&id='+id+'&tag='+tag,function (data){
                            window.location.href=window.location.href;
            });
	}
}
