...
<h1 class="title">
	<span>待处理&nbsp;&nbsp;&nbsp;
		<?php if($sign||$outrecord||$oddtime||$overtime||$leave||$hugh||$_SESSION['ADMIN_ID']=='106'){?>
...
<div class="pidding_5">
 ...
 <div class="pidding_5" >
  <table cellspacing="0" cellpadding="0" class="Admin_L">
    <tr>
    <?php if($hugh||$_SESSION['ADMIN_ID']==106){?>
    <th scope="col" class="T_title">
    ...
    ?>
    <tr class="Ls2" <?php if($lastdayred){?> style="color:red "<?php }?>>
      <?php if($hugh||$_SESSION['ADMIN_ID']==106){?>
      <td class="N_title"><input type="checkbox" id="<?=($lastdayred)?'red'.$key:'blue'.$key; ?>" name="hugh[]" value="<?=$val['id']?>" /></td>
      <?php }?>
      <td class="N_title"><?=$admin->getInfo($val['uid'],'real_name','pass')?></td>
      <td class="N_title"><?=$dep->getInfo($val['depId'],'name','pass')?></td>
      <td><?=$val['fromTime'].' '.$val['hour_s'].':'.$val['minute_s']."~".$val['toTime'].' '.$val['hour_e'].':'.$val['minute_e']?></td>
     ...
     <script language="javascript">
      ...
      <div id="ele<?=$val['id']?>" class="tigger">
	    点击查看
      </div>
