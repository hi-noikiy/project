...
//8月29号补班
$fillTime = $webdb->getValue("select totaltime from _web_record where card_id='".$info['card_id']."' and recorddate ='2015-08-29'",'totaltime');
//8月29号调休
$hugh_0829 = $webdb->getValue("select sum(totalTime) from _web_hugh where uid='{$info['id']}' and fromTime='2015-08-29' and depTag='2' and perTag='2' and manTag='2'", 'totalTime');
//8月29号总的上班时间
...
<div class="pidding_5">
    <div class="rs_l">
  	...
  	<table cellspacing="0" cellpadding="0" class="Admin_L">
	    ...
	    <tr class="Ls2">
	        <td class="N_title" colspan="9"><font color="red">正常上班时间:<?=floor($total/60)."小时".($total%60)."分钟"?></font></td>
	    </tr>
	    <?
    	foreach($arytmp as $key=>$val){
        	...
        	if($val) {
            	...
            	if($lt<$val['latetime']) 
            		$ctag = true;
	    ?>
	    <tr class="Ls2" <?if ($val['recorddate'] == '2015-08-29' || $val['recorddate'] == '2015-08-18' || $val['recorddate'] =='2015-08-20' ) echo 'style="color:blue;"'?>>
	        ...
	        <td class="N_title">
	            <?//调休
	            	if($val['recorddate'] == '2015-08-18'){
						if($work_0829){ //有补班
							//18号调休的宗时间
							$hugh_0818 = $webdb->getValue("select sum(totalTime) as totalTime  from _web_hugh where uid='{$info['id']}' and fromTime='2015-08-18' and depTag='2' and perTag='2' and manTag='2'", 'totalTime');
							//小于400分钟 说明下午没有调休
							if($hugh_0818*60 <= 300){
								$lt = $lt + 300;
								$work_0829 = $work_0829 - 300;		 
							}
	            	...
	            	if($key == '2015-08-20'){
	            		if($work_0829 && ($val['totaltime']!= '480')){
	        ...
	        <td class="N_title">
	            <?//请假
	                $le = $webdb->getValue("select sum(totalTime) as le from _web_leave where uid='$adid' and available='1' and manTag='2' and fromTime='".$val['recorddate']."'",'le');
	        ...
	        <td class="N_title">
		        <?//实际扣考勤
		        	$late=$val['latetime']-$outRecord-$lt;
    	...
    	<?
            	...
            	if($lt<$c)
            		$ctag = true;
        ?>
        <tr class="Ls2" <?if ($key == '2015-08-29' || $key == '2015-08-18' || $key =='2015-08-20' ) echo 'style="color:blue;"'?>>
