    <?php
		...
		$result = $leaveclass->getAnnual($uid,date('Y'));//今年年假
		$msg = '';
		if($result){
			$ny = $result['allTime']-$result['useTime'];
			$msg = "今年剩余{$ny}小时年假";
		}

		$md = date('md');
		if($md<'0301'){ //可以取2年年假时间
			$result1 = $leaveclass->getAnnual($uid,date('Y',strtotime('-1 year')));//去年年假
			if($result1){
				$ny1 = ($result1['allTime']-$result1['useTime']);
				$ny += $ny1;
