...
<h1 class="title">
 ...
 <div class="pidding_5">
     <div>
         ...
         ?>
         <br><font color="red">按公司规定：上班期间，出门时间超过15分钟的要扣考勤(参考时间为门禁卡时间)。</font>
     </div>
  <table cellspacing="0" cellpadding="0" class="Admin_L">
    ...
    <?foreach($arytmp as $key=>$val){
        ...
        if($val)
        {
            ...
            $outRecord=$webdb->getValue("select sum(totalM) totalM from _web_outrecord 
            where fromTime='".$val['recorddate']."' and uid='$id'  and available='1' and manTag='2'",'totalM');//公出时间
    ...
    <?
        ...
        else
        {
            ...
            $outRecord=$webdb->getValue("select totalM from _web_outrecord 
            where fromTime='".$val['recorddate']."' and uid='$id'  and available='1' and manTag='2'",'totalM');//公出时间
            $lt = acLateTime('',$key,$key,$adid);
            $c = 8*60;
            if($lt<$c)$ctag = true;
        ...
        <td class="N_title">
          ...
          <td class="N_title">
        <?php //公出
        echo ($outRecord)?(floor($outRecord['totalM']/60)."小时".($outRecord['totalM']%60)."分钟"):"&nbsp"
        ?>
        </td>
        <td class="N_title">
        <?php //实际扣考勤
        $late=480-$outRecord['totalM']-$lt;
        echo ($late>0)?(floor($late/60)."小时".($late%60)."分钟"):"0小时0分钟"
        ?>
        </td>
    </tr>
